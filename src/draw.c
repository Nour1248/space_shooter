...
{
    SDL_QueryTexture(entity->texture, NULL, NULL, &entity->rect.w, &entity->rect.h);
    SDL_RenderCopy(g_app.renderer, entity->texture, NULL, &entity->rect);
}

//
void blit_scaled(Entity *entity, int32_t w, int32_t h)
{
    entity->rect.w = w;
    entity->rect.h = h;
    SDL_RenderCopy(g_app.renderer, entity->texture, NULL, &entity->rect);
...
//
void blit_scaledAnimation(Animation *a, SDL_Rect *r)
{
    a->rect = *r;
...
//
bool is_outOfBounds(Entity *e)
{
    if (!(e->rect.x < g_WIDTH && e->rect.x > -(e->rect.w)) || !(e->rect.y < g_HEIGHT && e->rect.y > -(e->rect.h)))
        return true;
    else
        return false;
}
...
//
bool are_colliding(SDL_Rect *r1, SDL_Rect *r2)
{                                // thx mdn
...
//
void animate_player(Entity *entity)
{
    if (g_app.up)
    {
...
//
void blit_bullets(Entity **bulls)
{
    for (size_t i = 0; i < BULLETS_COUNT; i++)
        blit(bulls[i]);
}
...
//
void shoot(Entity **bulls, Entity *player)
{
    static size_t i = 0;
    if (g_app.fire)
    {
...
//
void animate_bullets(Entity **bulls)
{
    for (size_t i = 0; i < BULLETS_COUNT; i++)
    {
...
{
    char *meteors[31] = {"assets/meteorBrown_small2.png\0", "assets/meteorBrown_small1.png\0",
...
//
void blit_meteors(Entity **meteors)
{
    for (size_t i = 0; i < METEORS_COUNT; i++)
    {
...
char *g_explosionSheet[9] = {
    "assets/regularExplosion00.png", "assets/regularExplosion01.png", "assets/regularExplosion02.png",
    "assets/regularExplosion03.png", "assets/regularExplosion04.png", "assets/regularExplosion05.png",
    "assets/regularExplosion06.png", "assets/regularExplosion07.png", "assets/regularExplosion08.png"};

//
void animation(Animation *a, char **sheet, size_t sheet_size)
{
    if (g_clock < a->start + (a->delay * (sheet_size)))
    {
        a->texture = load_texture(sheet[a->idx]);
        SDL_RenderCopy(g_app.renderer, a->texture, NULL, &a->rect);
    }

    if (a->start + (a->delay * (a->idx + 1)) < g_clock && a->start != 0)
    {
        a->idx++;
    }
...
//
void play_explosionAnimation(void)
{
    for (size_t i = 0; i < ANIMATION_UNIT_COUNT; i++)
    {
...
void animate_meteors(void)
{
    ...
    for (size_t i = 0; i < METEORS_COUNT; i++)
    {
        ...
        {
            if (are_colliding(&g_meteors[i]->rect, &g_bullets[j]->rect) && g_meteors[i]->hp != 0)
            {
